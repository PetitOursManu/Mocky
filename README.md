...
- 🎞️ **Motion Ultra** — a project setting that storyboards each new screen, generates a series of ×3 or ×6 pictures for it in one shared style, and writes it as a motion-led page: living backgrounds, glass, display type, scroll reveals. See [docs/motion-ultra.md](docs/motion-ultra.md).
- 🖼️ **Infinite canvas** — a Stitch-like dotted board; pan/zoom, real-size resizable frames, Windows-style multi-select (click / Ctrl-click / marquee), arrange-to-grid, **Fit all** and **Zoom to the latest screen**. The framing is kept as an intention rather than as coordinates, so it recomputes itself when the window or a side panel changes size — and releases the view the moment you pan or zoom by hand.
